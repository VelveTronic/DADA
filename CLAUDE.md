...
- orders.staff_note is internal: column-revoked from authenticated (same pattern as price tiers). Customer/staff client code must never select('*') on orders — enumerate columns or use the typed row; staff_note round-trips via service role only. create_order takes an optional client_token uuid for idempotent retries; the bridge marks return boolean and false means the order was NOT in the expected state (log and alert, never assume success).
