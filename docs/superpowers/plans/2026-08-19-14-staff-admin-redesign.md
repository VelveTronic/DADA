...
2. **No inventory.** 库存/缺货预警数量/仅 N 箱 have no data. The real signal is `is_available` (断货) and `is_current_variant`. "缺货预警" surfaces become "断货商品" (is_available = false). Products table has NO 库存 column.
...
8. **Sidebar 今日 block + 订单 badge are real counts** (submitted / bridge_failed / 断货), read by StaffShell (server) with `head:true` count queries raced with the guard, passed to the sidebar as props. No polling.
...
- [ ] `StaffNavKey` gains `"categories"` (5-edit checklist, survey §6g): union, `NAV_PATH` (`/staff/categorias`), `NAV_ICON` (new 2×2-squares glyph in icons.tsx drawn on the house 20-grid, `ICON_PROPS`), gate list (ungated — any staff), `staff.nav.categories` 分类/Categorías. Relabel `staff.nav.users` 用户→客户 / Usuarios→Clientes (key stays `users`).
- [ ] StaffShell reads THREE `head:true` counts raced with the guard under one `perf.step("counts")`: orders where status=submitted; orders where status=bridge_failed; products where is_available=false (service-role NOT needed — orders/products session reads under staff RLS are already how /staff/pedidos reads). Pass `{submitted, bridgeFailed, unavailable}` to the sidebar.
- [ ] Sidebar lg=240px look per mockup: brand row (28px disc + DADA + 商家 badge `text-[10.5px] border border-border-strong rounded px-1 text-muted`), nav rows `h-[38px] rounded-lg px-3 gap-2.5 text-[13.5px]` active `bg-brand-soft text-brand-ink font-bold` + 18px icons; 订单 row badge `min-w-5 h-5 rounded-full bg-brand text-white font-num text-[11px] font-bold` showing `submitted` (hidden at 0); divider; 今日 block (label `text-[11px] font-semibold tracking-wide text-muted` + three rows label/number, bridge_failed row `text-brand-ink` when >0); footer user card (initial disc `bg-surface-dim`, name `text-[12.5px] font-semibold`, role label `text-[11px] text-muted`) — logout stays wherever it lives today (keep the affordance, restyle only).
- [ ] Icon rail (sm) + drawer (<sm) keep every documented contract (focus return, Escape, matchMedia close, NO filters on the bar). Mockup's ⌘K search: OUT.
- [ ] Gate. Fixture: sidebar at lg/sm/drawer with counts {5,1,6} and {0,0,0} (badge + 今日 rows hide at 0 where designed); active states per path; screenshots.
- [ ] Commit `feat(staff): admin sidebar per mockup — 今日 counts, categories entry, 客户 relabel`.
...
- [ ] RIGHT detail (selected or first): header name + `{n} 个商品 · 客户端排序第 {i} 位` + 重命名 form (zh + es FIELD inputs, save BTN) + 隐藏/显示 toggle form (with the copy from decision 5); `parent_label` shown read-only as 分组标签 when present; products-in-category list (read-only, thumb+name+codart, first 50, note when more); empty-category state. ＋新建一级分类 (header button per mockup) opens the create form card (zh+es name fields; explains hidden-by-default).
- [ ] i18n `staff.categories.*` zh+es (title, listHead, newButton, moveUp/moveDown aria, hiddenChip, rename, show/hide + confirmCopy, productsHead, groupLabel, empty states, results.* for every CATEGORY_ERRORS member + ok) + messages.test closed-list check.
- [ ] Gate. Fixture (real components, constant rows incl. tie sort_orders + a hidden one + zh-only name): move up/down re-orders and disables at edges; rename round-trip via inert action; hidden chip; selection; screenshots zh+es, lg + drawer width.
- [ ] Commit `feat(staff): category management — rail order, rename, hide, create, zero new RPCs`.
...
- [ ] Restyle to the mockup table look on the ADMIN card (12px radius, `#EDE9E5` hairline, header row `bg-field text-[11.5px] text-muted`, rows `min-h-16 hover:bg-[#FCFBFA]`): 商品 (44px thumb + name + `SKU {codart}` `font-num text-[11px] text-muted`) / **分类** (NEW: per-row `<select>` of active categories + 未分类, submits `setProductCategory` on change via a tiny client leaf OR a per-row form with 保存 — pick the house-consistent shape and justify) / 规格 (unitLabel) / 状态 (real chips: 在售=`is_available`, 断货, plus the current-variant badge as today) / 价格 n/6 (KEPT) / 操作 (the three existing toggles, restyled `BTN_QUIET`-family). NO 库存, NO 新建/导入 buttons; header sub-line = real counts (`共 {n} 个商品 · {m} 个分类`).
- [ ] Category filter chips row per mockup (全部 + top categories by count + a `<select>` overflow for the rest — 61 chips don't fit; decide and document), filter via `?cat=` erp_code like the customer page; search + pager kept.
- [ ] `setProductCategory(productId, categoryId|null)` in staff-products.ts (service-role after the CONVERGED shared staff assert — fold the two `assertStaff` copies into one exported helper in this task), validates category exists + active-or-hidden, revalidates productos + catalogo + buscar.
- [ ] i18n additions; gate; fixture (assignment select posts the right ids; chips filter; table at 1280 + drawer width); commit `feat(staff): products table per mockup with category assignment`.
...
- [ ] Keep the ENTIRE information architecture (tabs=QUEUE_TABS, per-order card with lines `<details>`, line-qty forms, confirm/cancel/requeue, bridge-failure box, result banners). Restyle: page header per mockup (title + sub explaining the REAL flow: 客户提交 → 确认 → 注入 ERP → 出单); tab strip → mockup filter chips (active `bg-ink text-white`, with real counts when cheap — the tabs already know their counts? if not, `head:true` per tab under one perf.step, capped at the 4 real tabs); order rows get the mockup's grid rhythm (单号 `font-num`, 客户/门店 stacked, 种类/件数 `tabular-nums`, chips right) while KEEPING the expandable detail beneath; actions restyled (确认=red solid, 取消=quiet, 重新入队=brand outline).
- [ ] Fix the HANDOFF debt: the `order_items` read gains `.order("order_id").order("sort_order").limit(1000)` + the truncation comment (mirror the customer fix).
- [ ] Gate; fixture (all 7 chip states, failure box, line edit form); commit `feat(staff): orders queue in the admin chrome — real statuses, bounded lines read`.
...
- [ ] KPI strip (4 cells, `font-num text-[34px]`): 今日订单 (created_at ≥ Madrid today — reuse `madridDay`/month helper family; add `madridDayStartIso` to orders.ts WITH tests if not derivable), 待确认 (submitted), 进行中 (confirmed+processing+injected), 断货商品 (products is_available=false). All `head:true` counts, one perf.step.
...
- [ ] 断货商品 card (wash `#FFF8F7`/`#FBE4E2` per mockup): products is_available=false, name+codart, first 6, 去处理 → productos. 待处理事项 card: bridge_failed orders (real, each linking the queue) + oldest submitted age; empty state 今日无待办.
