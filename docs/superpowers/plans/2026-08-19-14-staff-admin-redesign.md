...
> Shipped: cleanup `3690653` (ADMIN_TD converged — the two TH strings genuinely differ and stayed local; `readLoggedCount` is the single count logger, 14 migrated sites, emitted lines proven byte-identical over a 98-case differential; toggle focus ring `ring-offset-2` with the #fff default kept because the switch sits on ADMIN_CARD's white, not the wash; `profile.customerNo` N.º; `user-admin.ts:148` reworded) + final-review polish (M2 prose-token break, M3 formText doc names `lib/profile.ts`'s remaining copy, M4 order-card glyph+comment to N.º). Final cross-task review verdict SHIP: security gates all clean (supabase/, package.json, guards.ts zero diff over the whole range; every new write in its lane; assertStaff single-sourced; 12 RPC names = the accepted baseline exactly), one-off ledger complete, zero text-faint on the staff surface, i18n 421=421 with zero orphans, single-source invariants hold, customer surface touched only where promised. NOT done, deliberately open: the `cache()` unification of the three count readers (shell + queue chips + dashboard) — a design change, recorded in-code at both sites; and `/cuenta`'s log-blind `?? 0` head counts — handed to the HANDOFF.
